...
CONTRACT_ADDRESS = "0xDF2Dc2E4C400B896627EE210e05b0f8508bf3DCD"

# Contract ABI (simplified version of what would be generated from your Solidity contract)
CONTRACT_ABI = [
    {
        "inputs": [
    ...
    {
        "inputs": [{"internalType": "bytes32", "name": "examId", "type": "bytes32"}],
        "name": "exams",
        "outputs": [
            {"internalType": "string", "name": "title", "type": "string"},
            {"internalType": "uint256", "name": "date", "type": "uint256"},
...
def get_web3_contract():
    """Initialize and return web3 contract instance"""
...
def create_exam(request):
    if request.method == "POST":
        try:
            ...
            metamask_account = request.POST.get('metamask_account')
            
            if not metamask_account:
                messages.error(request, 'Please connect MetaMask')
            ...
            exam_data = {
                ...
                'duration': int(request.POST['examDuration']),
                'marks': int(request.POST['marks'])
            }

            # Create exam hash
            exam_hash = Web3.keccak(text=f"{exam_data['title']}{exam_data['date']}{exam_data['duration']}{exam_data['marks']}").hex()
            ...
            exam = Exam.objects.create(
                ...
                blockchain_hash=exam_hash
            )

            # Register exam on blockchain
            exam_id = Web3.keccak(text=f"{exam.id}{exam.title}").hex()
            tx_hash = contract.functions.registerExam(
                Web3.toBytes(hexstr=exam_id),
                exam.title,
                int(exam.date.timestamp()),
                exam.duration,
                exam.total_marks,
                Web3.toBytes(hexstr=exam_hash)
            ).transact({'from': metamask_account, 'gas': 1000000})
            ...
            while True:
                ...
                question_hash = Web3.keccak(
                    text=f"{question_data['text']}{''.join(question_data['options'])}{question_data['correct_answer']}"
                ).hex()
                ...
                question = Question(
                    ...
                    blockchain_hash=question_hash
                )
                mcqs.append(question)

                # Register question on blockchain
                question_id = Web3.keccak(text=f"{exam.id}{i}{question.text}").hex()
                contract.functions.registerQuestion(
                    Web3.toBytes(hexstr=question_id),
                    Web3.toBytes(hexstr=exam_id),
                    Web3.toBytes(hexstr=question_hash)
                ).transact({'from': metamask_account, 'gas': 1000000})
            ...
            messages.success(request, 'Exam created and stored in blockchain successfully!')
            return redirect('teacher_dashboard')
...
def verify_exam(request, exam_id):
    ...
    try:
        contract = get_web3_contract()
        exam_blockchain_id = Web3.keccak(text=f"{exam.id}{exam.title}").hex()
        
        # Get stored hash from blockchain
        stored_hash = contract.functions.exams(Web3.toBytes(hexstr=exam_blockchain_id)).call()[4]
        current_hash = exam.blockchain_hash
        
        # Compare hashes
        is_valid = stored_hash == Web3.toBytes(hexstr=current_hash)
        ...
        for i, question in enumerate(questions):
            question_id = Web3.keccak(text=f"{exam.id}{i}{question.text}").hex()
            q_stored_hash = contract.functions.questions(Web3.toBytes(hexstr=question_id)).call()[0]
            q_current_hash = question.blockchain_hash
            questions_valid.append(q_stored_hash == Web3.toBytes(hexstr=q_current_hash))

        return render(request, 'verify_exam.html', {
            'exam': exam,
            'is_valid': is_valid,
            'questions_valid': all(questions_valid),
            'verification_details': {
                'blockchain_id': exam_blockchain_id,
